...
- **Revise today's Pydantic material before the next session**
  - Instructor flagged that unrevised doubts are slowing pace; from next weekend, teaching will be uninterrupted with a dedicated Q&A window only.
- **Review the pushed code on GitHub**
  - All session code has been committed; study it ahead of tomorrow's live API call and agent walkthrough.
- **Practice Pydantic in Google Colab (beginners) or VS Code (experienced)**
  - Tomorrow's session moves to VS Code permanently; Colab recommended only for those who have never coded before.